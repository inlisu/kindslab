...
require_once $_SERVER['DOCUMENT_ROOT'].'/engine/modules/tech/includes/init.php';
	...
	$query = "SELECT sum(money),count(violation.id),
                     (SELECT user_id from dle_users where name='$u_name') as user_id 
              FROM `violation` 
              left join rules on rule_id = rules.id
              where date_paid='0000-00-00 00:00:00' 
                   and rules.user_id=(SELECT user_id from dle_users where name='$u_name')
                   ";
