...
require_once $_SERVER['DOCUMENT_ROOT'].'/engine/modules/tech/includes/init.php';
	...
	$query = "SELECT dle_users.user_id, dle_users.name, fg.name, fg.duration, fg.spend, fg.assignment,dle_users.money, blank.bl FROM `dle_users` left join (select name, user_id, duration, FLOOR((UNIX_TIMESTAMP(now())-UNIX_TIMESTAMP(time_start))/60) as spend, assignment from log_action left join common_action on common_action.id = task_id where time_end='0000-00-00 00:00:00') as fg on fg.user_id=dle_users.user_id left join (select user_id , FLOOR((UNIX_TIMESTAMP(now())-UNIX_TIMESTAMP(max(time_end)))/60) as bl from log_action group by user_id) as blank on blank.user_id=dle_users.user_id where dle_users.user_id in (select ch_id from parents where p_id = (select user_id from dle_users where name='$u_name_p'))";
